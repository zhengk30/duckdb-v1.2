...
idx_t FindStrInStr(const unsigned char *haystack, idx_t haystack_size, const unsigned char *needle, idx_t needle_size) {
	// NOTE: kernel for contains(haystack, needle)
	char needle_escaped[1024] = {0};
	char* p = needle_escaped;
	...
	// printf("strchr(needle, '\\') == NULL ? %d\n", strchr((const char*)needle, '\\') == NULL);
	printf("haystack=%s, needle_escaped=%s\n", haystack, needle_escaped);
	string haystack_obj(reinterpret_cast<const char *>(haystack), haystack_size);
	RE2 needle_obj(reinterpret_cast<const char *>(needle_escaped));
	bool contains_needle = RE2::PartialMatch(haystack_obj, needle_obj);
	if (!contains_needle) {
		return DConstants::INVALID_INDEX;
	} else {
		auto location = memchr(haystack, needle[0], haystack_size);
		return reinterpret_cast<const unsigned char *>(location) - haystack;
	}
