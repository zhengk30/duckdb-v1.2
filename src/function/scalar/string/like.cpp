...
bool LikeOperatorFunction(const char *s, idx_t slen, const char *pattern, idx_t plen) {
	...
	idx_t pattern_idx = 0;
	char pattern_buf[1024] = {0};  // TODO: max buffer size to be changed
	for (idx_t i = 0; i < plen; i++) {
		if (pattern[i] == '%') {
			pattern_buf[pattern_idx++] = '.';
			pattern_buf[pattern_idx++] = '*';
		} else if (pattern[i] == '_') {
			pattern_buf[pattern_idx++] = '.';
		} else {
			pattern_buf[pattern_idx++] = pattern[i];
		}
	}
	string s_obj(s, slen);
	RE2 pattern_obj(pattern_buf);
	return RE2::FullMatch(s_obj, pattern_obj);
}
